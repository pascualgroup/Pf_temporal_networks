...
## @knitr Initialize
source('functions.R')
prep.packages(c("tidyverse","magrittr","data.table","igraph","Matrix","dplyr","cowplot",'sqldf', 'aod'))
...
for (i in 1:nrow(experiments)){
  ...
  x <- get_modularity_results(ps,scenario,exp,1,cutoff_prob,folder = paste('/media/Data/PLOS_Biol/Results/',ps,'_',scenario,'/',sep=''))
  if (!is.null(x)){
    x <- subset(x, layer<=6)
    x$modularity_exp <- modularity_exp
...
module_results_simulations <- as_tibble(module_results_simulations)
